...
class YServo(YFunction):
    ...
    def __init__(self, yctx: YAPIContext, func: str):
        super().__init__(yctx, 'Servo', func)
        # --- (YServo constructor)
    ...
    def FindServo(cls, func: str) -> YServo:
        ...
        """
        return cls.FindServoInContext(YAPI, func)
    ...
    def FindServoInContext(cls, yctx: YAPIContext, func: str) -> YServo:
        ...
        @return a YServo object allowing you to drive the RC servo motor.
        """
        obj: Union[YServo, None] = yctx._findInCache('Servo', func)
        if obj:
            return obj
        return YServo(yctx, func)
    ...
    def FirstServo(cls) -> Union[YServo, None]:
        ...
        @return a pointer to a YServo object, corresponding to
                ...
                if there are none.
        """
        return cls.FirstServoInContext(YAPI)
    ...
    def FirstServoInContext(cls, yctx: YAPIContext) -> Union[YServo, None]:
        ...
        @return a pointer to a YServo object, corresponding to
                ...
                if there are none.
        """
        hwid: Union[HwId, None] = yctx._firstHwId('Servo')
        if hwid:
            return cls.FindServoInContext(yctx, hwid2str(hwid))
        return None
    ...
    def nextServo(self) -> Union[YServo, None]:
        ...
        try:
            next_hwid = self._yapi._nextHwId('Servo', self.get_hwId())
        except YAPI_Exception:
            pass
        if next_hwid:
            return self.FindServoInContext(self._yapi, hwid2str(next_hwid))
        return None
    ...
    async def get_position(self) -> int:
        ...
        """
        json_val: Union[int, None] = await self._fromCache("position")
        if json_val is None:
    ...
    async def get_range(self) -> int:
        ...
        """
        json_val: Union[int, None] = await self._fromCache("range")
        if json_val is None:
    ...
    async def get_neutral(self) -> int:
        ...
        """
        json_val: Union[int, None] = await self._fromCache("neutral")
        if json_val is None:
    ...
    async def get_move(self) -> YMove:
        json_val: Union[YMove, None] = await self._fromCache("move")
        if json_val is None:
    ...
    async def get_positionAtPowerOn(self) -> int:
        ...
        """
        json_val: Union[int, None] = await self._fromCache("positionAtPowerOn")
        if json_val is None:
